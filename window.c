...
{
	if (wp == NULL)
		wp = wheadp;					/* wheadp is never == NULL */
	curwp = wp;
	curbp = wp->w_bufp;
	return TRUE;
}
...
WINDOW *wpopup(void)						/* rewritten by pm */
{
	...
	if (wheadp == mlwp)
		return NULL;
	wp = predwp(mlwp);
	if (wp->w_ntrows > term.t_nrow / 4 && lsplitwind(wp))
		wp = predwp(mlwp);
	return wp;
}
